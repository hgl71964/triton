...
def _compile_kernel(dir, signature, kernel_name, out_name, out_path, num_warps,
                    grid, kernel_path):
...
def link_aot_kernels(dir):
    ...
    subprocess.run([sys.executable, linker_path] + h_files + ["-o", "kernel"],
                   check=True,
                   cwd=dir)
...
def test_compile_link_matmul_no_specialization():
    ...
    compile_aot_kernel_no_specialization(tmp_dir, kernel_path, dtype, BM, BN,
                                         BK)
...
def test_compile_link_matmul():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        kernel_path = write_triton_kernels(tmp_dir, kernel_src,
                                           kernel_utils_src)
        compile_aot_kernels(tmp_dir,
                            kernel_path,
                            dtype,
                            BM,
                            BN,
        ...
        # initialize test data
        a, b, a_path, b_path, c_path = generate_matmul_test_data(
            tmp_dir, M, N, K)
...
test_compile_link_autotune_matmul()
