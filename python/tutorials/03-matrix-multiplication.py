...
def matmul_kernel(
    ...
    a_ptrs = a_ptr + (offs_am[:, None] * stride_am + offs_k[None, :] * stride_ak)  # got boardcast
...
no_l2 = matmul_no_L2(a, b, "leaky_relu")
...
if torch.allclose(no_l2, torch_output, atol=1e-2, rtol=0):
    print("✅ Triton No L2 and Torch match")
else:
    print("❌ Triton and Torch differ")
...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        ...
        line_vals=['cublas', 'triton', 'triton_no_L2'],
        # Label name for the lines
        line_names=["cuBLAS", "Triton", "Triton No L2"],
        # Line styles
...
def benchmark(M, N, provider):
    ...
    if provider == 'cublas':
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: torch.matmul(a, b),warmup=100, rep=100,  quantiles=quantiles)
    if provider == 'triton':
    ...
    if provider == 'triton_no_L2':
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: matmul_no_L2(a, b),warmup=100, rep=100,  quantiles=quantiles)
