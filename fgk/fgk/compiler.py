...
def ttir_to_ttgir(mod, num_warps, num_ctas, target):
    pm = ir.pass_manager(mod.context)
    pm.enable_debug()
    pm.add_convert_triton_to_tritongpu_pass(num_warps, 32, num_ctas,
                                            target.capability)
    pm.run(mod)
    return mod


def optimize_ttgir(mod, num_stages, num_warps, num_ctas, target, cluster_info,
                   enable_warp_specialization, enable_persistent,
                   optimize_epilogue):
    ...
    if ws_enabled:
        ...
        pm.add_tritongpu_pipeline_pass(num_stages, num_warps, num_ctas,
                                       capability)
...
def ttgir_to_llir(mod, extern_libs, target, tma_infos):
    ...
    if _is_cuda(target):
        return translate_triton_gpu_to_llvmir(mod, target.capability,
                                              tma_infos, runtime.TARGET.NVVM)
    else:
        return translate_triton_gpu_to_llvmir(mod, 0, TMAInfos(),
                                              runtime.TARGET.ROCDL)
...
def llir_to_ptx(mod: Any,
                target: CudaTargetDescriptor,
                ptx_version: int = None) -> str:
    '''
...
def make_hash(fn_cache_key, target, env_vars, device_backend, **kwargs):
    ...
    key = f"{fn_cache_key}-{version_key}-{''.join(signature.values())}-{configs_key}-{constants}-{num_warps}-{num_stages}-{num_ctas}-{num_stages}-{enable_warp_specialization}-{enable_persistent}-{debug}-{target}-{env_vars_list}"
...
def compile(fn, fn_name, fn_cache_key, **kwargs):
    ...
    ids_of_const_exprs = tuple(fn.constexprs)
    ids = {
